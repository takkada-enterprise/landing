...
export function readReferencedSlugs(appRoot) {
  ...
  if (existsSync(published)) {
    // Generated by the app from the DEPLOYED manifest. Every slug in it is a
    // link a shipped app version opens, internal or not, so nothing is filtered
    // here: an internal id reaching this file is precisely the KD-7 breach the
    // contract has to fail on.
  ...
  if (existsSync(placements)) {
    ...
    for (const record of parsed?.placements ?? []) {
      // placements.json is an audit of where help sits INSIDE the app. For the
      // twenty-one public topics the topic id is also the public slug, and
      // reading it as one is what catches an app topic the site never
      // published. KD-7 is the exception in the other direction: "Entries
      // waiting for approval" is explicitly allowed an in-app explanation and
      // is explicitly forbidden a public page, so an in-app placement for it is
      // the contract working, not a breach.
      //
      // An explicit `publicSlug` is always a public claim and is never
      // filtered, so an app that really did try to publish the internal topic
      // still fails — as does one that lists it in published_guides.json above.
      if (record?.publicSlug === null) continue;
      const declared = record?.publicSlug;
      const slug = declared ?? record?.topicId;
      if (!slug) continue;
      if (declared === undefined && INTERNAL_TOPIC_IDS.has(slug)) {
        inAppOnly.add(slug);
        continue;
      }
      slugs.add(slug);
  ...
  return { slugs: [...slugs].sort(), sources, inAppOnly: [...inAppOnly].sort() };
}
...
async function main() {
  ...
  for (const slug of inAppOnly ?? []) {
    process.stdout.write(
      `checkGuideContract: note — the app places help for "${slug}" in-app only; it is ` +
        'internal (KD-7) and gets no public page. The placement record declares no ' +
        'publicSlug, so record "publicSlug": null there to make that explicit.\n'
    );
  }
