...
meta_title: "Pending Orders Report for Distributors"
meta_description: "A pending orders report distributor teams use lists every sales order not yet invoiced, then feeds Today's load and the Dispatch Load sheet."
primary_keyword: "pending orders report distributor"
date: "2026-08-26"
updated: "2026-08-26"
author: "founder"
category: "Tally Mobile"
excerpt: "Gupta Stores' 14 cartons are a Sales Order in Tally and still waiting for an invoice. Pending orders puts every such line in one list. Today's load and Dispatch read the same pile for the next van."
---
...
The [Salesman app for Tally](/salesman-app-tally) is where many of those orders were booked. [Sales order on mobile](/sales-order-on-mobile) is that booking, written back as an ordinary Sales Order. Pending is what you open after, when you need the ones that have not become invoices yet.
...
[Create beats and assign a salesman](/blog/create-beats-and-assign-salesman-mobile/) is what put Raju on Station Road so his slice has an owner. The pending list does not assign anyone. It reports the open Sales Orders. Assignment is the beat. The report is the leftover bill work.

A [salesman-wise sales report](/blog/salesman-wise-sales-report-tally/) still reads what was booked. Pending reads what was booked and not yet billed.
...
Shops on Station Road with nothing pending stay on the beat and do not appear on Today's load. Raju can still visit them. The load is the pending pile for the window, not the full dealer list. [What a beat is in distribution sales](/blog/what-is-a-beat-in-distribution-sales/) is the named list. Today's load is the open orders on that list that still fall inside three days.
...
[Mark orders delivered without touching Tally](/blog/mark-orders-delivered-without-touching-tally/) is the afternoon of this same sheet. The boy ticks Mark delivered at Gupta Stores. That tick does not invoice. The Sales Order stays open. Pending still holds it until the office writes the bill.
...
Closing pending means invoicing the Sales Order. The [field order collection app](/blog/field-order-collection-app-tally/) wrote the order at the counter, or the dealer wrote it on a [customer self-order link](/blog/customer-self-order-link-for-distributors/). Either way the books hold a Sales Order until someone bills it.
